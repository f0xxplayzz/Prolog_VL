...
scompare([],_).
scompare([H|T],List2):- maplist(scompare_elem(H),List2), scompare(T,List2).
...
scompare_elem(X=A,Y=B):- X == Y,A == B; not(X == Y).
...
compare_and_concat(A,B,Result):- scompare(A,B), append(A,B,Temp), sort(Temp,Result).
...
not_true_with(eq(X,Y),Result):- 
    ...
    constructable_with(Y,B),
    compare_and_concat(A,B,Result).
not_true_with(nor(X,Y),Result):-constructable_with(or(X,Y),Result).
not_true_with(nand(X,Y),Result):-constructable_with(and(X,Y),Result).
...
constructable_with(eq(X,Y),Result):- 
    ...
    not_true_with(Y,B),
    compare_and_concat(A,B,Result).
constructable_with(nor(X,Y),Result):-not_true_with(or(X,Y),Result).
constructable_with(nand(X,Y),Result):-not_true_with(and(X,Y),Result).
